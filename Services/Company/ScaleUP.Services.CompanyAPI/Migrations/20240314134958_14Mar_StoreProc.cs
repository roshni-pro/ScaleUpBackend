...
ALTER PROCEDURE [dbo].[spGetCurrentNumber] 
--Declare
	@EntityName nvarchar(100) ='Company',
	@companytype nvarchar(100) ='Anchor'
...
--SAVE TRANSACTION GetNextSequence;
 --BEGIN TRY
	...
	select @NumberConfigId = cc.Id,
		...
		where EntityId = @entityId and CompanyType = @companytype--and StateId = @StateId
	...
	update EntitySerialMasters set NextNumber = @number +1 where Id = @NumberConfigId and CompanyType = @companytype
