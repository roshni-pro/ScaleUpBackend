...
{
    /// <inheritdoc />
    public partial class _12092024_SP_GetOutstandingTransactionsList : Migration
    {
        /// <inheritdoc />
...
Begin
	...
	select    tmp.*
			...
			, cast(cast(abs(ISNULL(x.PaymentAmount, 0)) as decimal(18,2)) as float) PaymentAmount
	...
	outer apply 
	(
		select   SUM(CASE WHEN h.Code = 'Payment' THEN ROUND(ad.Amount,2) ELSE 0 END )  PaymentAmount 
	...
	outer apply(	
		...
		And pats.IsActive=1 And pats.IsDeleted=0  ---Change
	)pats
	outer apply(
