...
{
    /// <inheritdoc />
    public partial class _04Sep2024_sp2 : Migration
    {
        /// <inheritdoc />
...
Begin
	...
	select   i.Id as InvoiceId
		...
		INNER JOIN TransactionTypes tt with(nolock) on ats.TransactionTypeId = tt.Id  and tt.Code in ('OrderPlacement','OverdueInterest','PenaltyCharges','OrderPayment')
		...
		OUTER APPLY (
			...
			INNER JOIN TransactionTypes xtt with(nolock) on xats.TransactionTypeId = xtt.Id  and xtt.Code in ('OrderPlacement','OverdueInterest','PenaltyCharges','OrderPayment')
	...
	outer apply 
		(
			...
			inner join TransactionTypes t  with(nolock) on at1.TransactionTypeId = t.Id and t.Code in ('OrderPlacement','OverdueInterest','PenaltyCharges','OrderPayment')
		...
		WHERE (ISNULL(@LoanAccountId, 0) = 0 OR i.LoanAccountId = @LoanAccountId)
			  ...
			  AND (
					 ...
					 OR (@Status = 'Overdue' AND i.Status = 'Overdue' AND ats.DueDate >= @FromDate AND CAST(ats.DueDate as date) <= @ToDate)
					  OR (@Status = 'Delinquent' AND i.Status = 'Delinquent' AND ats.DueDate >= @FromDate AND CAST(ats.DueDate as date) <= @ToDate)
...
GO
	...
	outer apply(
			...
			inner join dbo.GetTransactionTypeId() t on t.id =a.TransactionTypeId
			where LoanAccountId=@LoanId and s.Code='Overdue' and a.IsActive=1 and a.IsDeleted=0 
			--AND A.InvoiceNo NOT IN ('MP242540547')
		) Overdue
...
GO
	...
	CREATE OR ALTER proc [dbo].[GetLoanAccountSummary]
	--declare
