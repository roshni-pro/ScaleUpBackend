...
{
    /// <inheritdoc />
    public partial class _03Aug2024_sp : Migration
    {
        /// <inheritdoc />
...
CREATE OR ALTER procedure [dbo].[GetBusinessLoanAccountList]
	...
	,lc.DisbursalAmount as loan_amount
	...
	inner join LoanAccountCredits lc with(nolock) on lc.LoanAccountId = la.Id
	...
	(@isDSA=0 and not exists(select 1 from @leadIds c where LA.LeadId = c.intValue))) -- or  not exists(select 1 from @leadIds)
	--and ((@Role in ('AYEFIN','MASFIN') and LA.NBFCIdentificationCode = @Role ) or @Role not in ('AYEFIN','MASFIN'))
	and (la.NBFCCompanyId=@NbfcCompanyId or @NbfcCompanyId=0)
	where 
