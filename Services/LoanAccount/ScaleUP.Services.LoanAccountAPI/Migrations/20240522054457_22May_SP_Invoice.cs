...
{
    /// <inheritdoc />
    public partial class _22May_SP_Invoice : Migration
    {
        /// <inheritdoc />
...
CREATE OR ALTER Procedure [dbo].[GetCompanyInvoicesCharges]
	...
	--insert into @AnchorId values(2)
		select 
		round(SUM(case when cd.InvoiceTransactionType = 1 then TotalAmount else 0 end),2) as ProcessingFee,
		round(SUM(case when cd.InvoiceTransactionType = 2 then TotalAmount else 0 end),2) as InterestCharges,
		round(SUM(case when cd.InvoiceTransactionType = 3 then TotalAmount else 0 end),2) as OverDueInterest,
		round(SUM(case when cd.InvoiceTransactionType = 4 then TotalAmount else 0 end),2) as PenalCharges,
		round(SUM(case when cd.InvoiceTransactionType = 5 then TotalAmount else 0 end),2) as BounceCharges,
		round(SUM(cd.TotalAmount),2) as TotalTaxableAmount,
		Round(SUM(cd.TotalAmount) * CAST(18 as float)/100,2) as TotalGstAmount,
		Round((SUM(cd.TotalAmount) + (SUM(cd.TotalAmount) * (CAST(18 as float)/100))),2) as TotalInvoiceAmount
		from CompanyInvoices ci with(nolock)		
...
from  AccountTransactions a with(nolock) 
 ...
 begin
   ...
   Insert into CompanyInvoiceDetails(CompanyInvoiceId,AccountTransactionId,InvoiceTransactionType,InvoiceAmt
    ...
    Select SCOPE_IDENTITY(),a.AccountTransactionId,
