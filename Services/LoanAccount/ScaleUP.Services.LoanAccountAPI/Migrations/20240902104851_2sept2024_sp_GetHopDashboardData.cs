...
{
    /// <inheritdoc />
    public partial class _2sept2024_sp_GetHopDashboardData : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            if (OBJECT_ID('tempdb..#TempAccountTransactionDetail') is not null)
				...
				,cast(a.DisbursementDate as date) as disbursementDate
				,l.ProductType,ProductId,l.AnchorCompanyId,l.AnchorName,l.CityName
				,cast(a.Created as date) as AccountTransactionCreatedDate into #TempAccountTransactionDetail
                    ...
                    where((d.TransactionDetailHeadId != 1 and cast(d.TransactionDate as date) = cast(@currentDate as date)) or(d.TransactionDetailHeadId = 1 and(cast(a.Created as date) = cast(@currentDate as date))))
				...
				Begin
                    INSERT INTO HopDashboardData(ProductType, ProductId, AnchorCompanyId, AnchorName, CityName, LTDUtilizedAmount, PrincipleOutstanding, InterestOutstanding, PenalOutStanding, OverdueInterestOutStanding, ODPrincipleOutstanding, ODInterestOutstanding, ODPenalOutStanding, ODOverdueInterestOutStanding, ScaleupShareAmount, DisbursementAmount, DisbursementDate, TransactionDate, Created, CreatedBy, LastModified, LastModifiedBy, Deleted, DeletedBy, IsActive, IsDeleted)

                    select
                    ProductType, ProductId, AnchorCompanyId, AnchorName, CityName,
                    ...
                    --scaleup share amount--
					,isnull(x.DisbursementAmount, 0),isnull(x.DisbursementDate, null),@currentDate TransactionDate, @currentDate,'system',null,'system','','',1,0
					...
					outer apply
                        ...
                        (
                            ...
                            select a.OrderAmount as DisbursementAmount,cast(a.Created as date) DisbursementDate
                            from AccountTransactions a

                            inner join TransactionTypes t with(nolock) on t.Id = a.TransactionTypeId

                            where a.IsActive = 1 and a.IsDeleted = 0 and a.LoanAccountId = ac.LoanAccountId and t.Code = 'Disbursement'

                            and cast(a.Created as date)= cast(@currentDate as date)
						) x

                    group by ProductType, ProductId, AnchorCompanyId, AnchorName, CityName, x.DisbursementAmount,x.DisbursementDate
                end
        end ";
