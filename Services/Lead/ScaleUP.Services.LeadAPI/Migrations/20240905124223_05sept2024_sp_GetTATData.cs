...
Begin
		...
		select l.Id,l.Created initiateDate,lp.LastModified as LastModifiedDate,l.SubmittedDate as SubmittedDate,lp.ActivityMasterName,lp.SubActivityMasterName,lp.IsActive,lp.IsDeleted,lp.IsCompleted,lp.IsApproved
	   ,l.Status,l.ProductId,l.ProductCode,lo.Created as SendToLosDate,lo.Status as LeadOfferStatus,lo.LastModified as LeadOfferLastModifiedDate
	   into #LeadActivityPrgress  from Leads l with(nolock)
		...
		inner join LeadOffers lo on lo.LeadId = l.Id and lo.IsActive=1 and lo.IsDeleted=0
  ...
  if(OBJECT_ID('tempdb..#temp') is not null)
   drop table #temp  	   
	  SELECT l.Id
	  ,l.Status,l.ProductId,l.ProductCode,l.initiateDate initiateDate,isnull(l.LeadOfferStatus,'') LeadOfferStatus
	  ,max(l.SubmittedDate) SubmittedDate
	  ...
	  ,case when (l.ActivityMasterName in('KYC','PersonalInfo','BusinessInfo','Bank Detail') and l.IsApproved=1 and l.IsCompleted =1  and l.IsApproved =1) then max(l.LastModifiedDate)
	  ...
	  ,case when isnull(l.LeadOfferStatus,'') = 'OfferGenerated' then max(l.LeadOfferLastModifiedDate) else null end as OfferDate
	  --, OfferDate
	  ...
	  group by l.Id,l.initiateDate,l.Status,l.ProductId,l.ProductCode,l.LastModifiedDate,l.IsCompleted,l.ActivityMasterName,l.SubActivityMasterName,l.IsApproved,l.SendToLosDate,l.LeadOfferStatus,l.LeadOfferLastModifiedDate
	if(OBJECT_ID('tempdb..#temp1') is not null)
   drop table #temp1 
	 select tt.Id,tt.initiateDate,tt.ProductCode,tt.ProductId,tt.Status,max(tt.SubmittedDate) SubmittedDate
    ...
    END AS AgreementAcceptDate
	 into #temp1
	 from #temp tt
	 group by tt.Id,tt.initiateDate,tt.ProductCode,tt.ProductId,tt.Status,SendToLosDate,OfferDate
